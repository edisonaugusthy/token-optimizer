...
export function filterReadOutput(rawOutput: string, workingDirectory: string): ReadFilterResult {
  ...
  if (rawOutput.includes("<type>directory</type>")) {
    ...
    // For file reads: keep full content, just clean up blank lines
    // (no content compression — the LLM needs complete files to avoid re-reads)
 ...
 * Keeps all content intact, just removes whitespace bloat.
 */
function deduplicateBlankLines(output: string): string {
